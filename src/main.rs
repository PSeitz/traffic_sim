...
//     fn draw(&mut self, ctx: &mut Context) -> GameResult<()> {
//         graphics::clear(ctx);
//         graphics::set_color(ctx, graphics::Color::from_rgba(128, 128, 128, 128))?;
//         graphics::circle(ctx,
//                          DrawMode::Fill,
...
struct Actor {
    ...
    // for players and cars, it is the actual hit points.
    life: f32,
}
...
/// Note that this *could* create cars outside the
/// bounds of the playing field, so it should be
/// called before `wrap_actor_position()` happens.
fn create_cars(num: i32, exclusion: Point2, min_radius: f32, max_radius: f32) -> Vec<Actor> {
    ...
    let new_cars = |_| {
        ...
        car.pos = exclusion + vec_from_angle(r_angle) * r_distance;
        car.velocity = random_vec(MAX_ROCK_VEL);
        car
    };
    (0..num).map(new_cars).collect()
}
...
impl Assets {
    ...
    fn actor_image(&mut self, actor: &Actor) -> &mut graphics::Image {
        match actor.tag {
            ...
            ActorType::Car => &mut self.car_image,
            ActorType::Road => &mut self.shot_image,
        }
    }
}
